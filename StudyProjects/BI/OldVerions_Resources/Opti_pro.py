...
def optimizeIt():
    ...
    for tries in range(3):
        node_num = input("Enter number of nodes: ")
        if type(node_num) == ""
    ...
    for elem in range(len(d)):
        ...
        for n in range(1, 100000, 1):
            f_test = 0.00001*n
            results.append(getf(f_test, d[elem], np.absolute(Q[0][elem])))

        
        for result in results:
            if result >= 0.9999 and result <= 1.0001:
                n_val = results.index(result)
                f_val = 0.00001*(n_val+1)
                f_temp.append(round(f_val, 5))
                break
...
def findSum(i, d, L, Q, v, f, delta_h, delta_q):
    ...
    for elem in range(len(d)):
        ...
        for n in range(1, 100000, 1):
            f_test = 0.00001*n
            results.append(getf(f_test, d[elem], np.absolute(Q[i+1][elem])))
