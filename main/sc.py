...
class Main(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def dl_perks(self):
        ...
        surv_perks_table, killer_perks_table = surv_perks_table_raw.find('tbody').find_all(
            'tr'), killer_perks_table_raw.find('tbody').find_all('tr')
...
class MessageDialog(QtWidgets.QDialog, Ui_MessageTemplate):
    ...
    def dialog_message(self, source):
        if source == 'Data_download':
            self.msg_lbl.setText(
                "Wasn't able to download data. Check the internet connection and try again (interruption may damage existing files, remember to rerun the program after resolving problems).")
